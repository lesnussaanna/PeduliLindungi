...
<div align="center p-3">
		<h5 ><b>Formulir Tes Antigen</b></h5>
   </div>
	<hr class="my-2">
	<br>
	<form method="post">
		<table class="table table-borderless">
      	...
      	<input class="form-control form-control-md mb-3" type="text" name="no_pengenal" placeholder="NIK sesuai KTP" aria-label=".form-control-lg example" required>
			...
			<tr>
				<h6>Status Antigen</h6>
				<select class="form-select form-select-md mb-3" aria-label=".form-select-lg example" name="status_antigen" required>
  ...
  <option value="Negatif">Negatif</option>
      	...
      	<input class="form-control form-control-md mb-3" type="date" id="date" name="tanggal_antigen" required>
			</tr>
		</table>
  <div class="container">
    ...
    if($hitungcocok==0) {?>
      <div class="alert alert-danger text-center" role="alert">Nama dan NIK tidak sesuai.</div>
        ...
        if(strtoupper($nama_lengkap) == strtoupper($ambildata['nama_lengkap'])){
			...
			if ($no_pengenalada >= 1){
     		 $koneksi->query("UPDATE tes_covid SET status_antigen='$_POST[status_antigen]',tanggal_antigen='$_POST[tanggal_antigen]' WHERE no_pengenal='$no_pengenal'");
      				echo "<script>alert('Data Disimpan!');</script>";
    		}else{
        $koneksi->query("INSERT INTO tes_covid (no_pengenal, nama ,status_antigen, tanggal_antigen) 
            VALUES('$no_pengenal','$nama_lengkap','$_POST[status_antigen]','$_POST[tanggal_antigen]')");
        echo "<script>alert('Data Disimpan!');</script>";}
