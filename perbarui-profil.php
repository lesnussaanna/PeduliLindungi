...
<div class="container">
  ...
  if (isset($_POST['simpan'])){
    ...
    $emaillama = $_SESSION['user']['email'];
    $emailbaru = $_POST['email'];
    $nopelama = $_SESSION['user']['no_hp'];
    $nopebaru = $_POST['nope'];
    $tgl = $_POST['tanggal_lahir'];
    $tanggal = date('Y-m-d', strtotime($tgl));


    $cekdata=$koneksi->query("SELECT * FROM kewarganegaaan WHERE no_pengenal = '$_POST[no_pengenal]'");
...
if($hitungcocok==0) {?>
  ...
  if(strtoupper($_POST['nama_lengkap']) == strtoupper($ambildata['nama_lengkap'])){
    if ($emaillama == $emailbaru && $nopelama == $nopebaru){
      $koneksi->query("UPDATE data_akun SET nama_lengkap='$_POST[nama_lengkap]',no_pengenal='$_POST[no_pengenal]',tanggal_lahir='$tanggal',status_kewarganegaran='$_POST[status_kewarganegaran]' WHERE id_data='$id'");
      echo "<script>alert('Data Disimpan!');</script>";
      echo "<meta http-equiv='refresh'content='0.5;url=changeses.php'>";
    }
    elseif($emaillama == $emailbaru && $nopelama != $nopebaru){
      $ambil = $koneksi->query("SELECT no_hp FROM data_akun WHERE no_hp='$nopebaru'");
      $noada = $ambil->num_rows;
      if ($noada >= 1){
        echo "<script>alert('No. HP telah Terdaftar, Masukan Ulang Data!');</script>";
      }
      else{
        $koneksi->query("UPDATE data_akun SET nama_lengkap='$_POST[nama_lengkap]',no_pengenal='$_POST[no_pengenal]',tanggal_lahir='$tanggal', status_kewarganegaran='$_POST[status_kewarganegaran]',no_hp='$nopebaru' WHERE id_data='$id'");
        echo "<script>alert('Data Disimpan!');</script>";
        echo "<meta http-equiv='refresh'content='0.5;url=changeses.php'>";    
      }   
    }
    elseif($emaillama != $emailbaru && $nopelama == $nopebaru){
      $ambil = $koneksi->query("SELECT email FROM data_akun WHERE email='$emailbaru'");
      $emailada = $ambil->num_rows;
      if ($emailada >= 1){
        echo "<script>alert('Email telah Terdaftar, Masukan Ulang Data!');</script>";
      }
      else{
        $koneksi->query("UPDATE data_akun SET nama_lengkap='$_POST[nama_lengkap]',no_pengenal='$_POST[no_pengenal]',tanggal_lahir='$tanggal',status_kewarganegaran='$_POST[status_kewarganegaran]',email='$emailbaru' WHERE id_data='$id'");
        echo "<script>alert('Data Disimpan!');</script>";
        echo "<meta http-equiv='refresh'content='0.5;url=changeses.php'>";  
      }   
    }
    else{
      $ambilnope = $koneksi->query("SELECT no_hp FROM data_akun WHERE no_hp='$nopebaru'");
      $ambilmail = $koneksi->query("SELECT email FROM data_akun WHERE email='$emailbaru'");
      $hitungnope = $ambilnope->num_rows;
      $hitungmail = $ambilmail->num_rows;

      if ($hitungnope >= 1 || $hitungmail >= 1) {
        echo "<script>alert('No. HP atau Email telah ada!');</script>";
      }
      else{
        $koneksi->query("UPDATE data_akun SET nama_lengkap='$_POST[nama_lengkap]',no_pengenal='$_POST[no_pengenal]',tanggal_lahir='$tanggal',status_kewarganegaran='$_POST[status_kewarganegaran]',email='$emailbaru',no_hp='$nopebaru' WHERE id_data='$id'");
        echo "<script>alert('Data Disimpan!');</script>";
        echo "<meta http-equiv='refresh'content='0.5;url=changeses.php'>";
      }
    } 
...
<div class="row mb-3 mt-5">
    ...
    <ul class="nav nav-pills flex-column mb-auto">
      ...
      <li>
        <a href="kotak-masuk.php" class="nav-link link-primary">
    ...
    <hr class="mt-5">
      ...
      <div class="col-md-8 themed-grid-col">
          ...
          <form method="post">
      		<span class="fs-4" ><strong>Profil</strong></span>
           <span class="ms-0" style="padding-left: 25rem;"> <a href="profil.php" class="btn btn-outline-primary" ><b>Batal</b></a>&nbsp; &nbsp;<button name="simpan" class="btn btn-primary"><b>Simpan</b></button></span>
           <hr>
           
           <div class="p-3">
           <ul class="navbar-nav">
            <li class="pb-1"><dl>
              <dt>Identitas Akun</dt>
              <dd><select class="form-select form-select-md" name="status_kewarganegaran"value="<?php echo($_SESSION['buyer']['status_kewarganegaran']); ?>" aria-label=".form-select-lg example" required>
  <option disabled>Pilih Identitas Akun</option>
  ...
  <option value="Warga Negara Asing">Warga Negara Asing</option>
            ...
            <li class="pb-1"><dl>
              <dt>Nama Lengkap</dt>
              <dd><input class="form-control form-control-md" type="text" name="nama_lengkap" value="<?php echo $_SESSION['user']['nama_lengkap']; ?>" required></dd>
            </dl></li>
              <li class="pb-1"><dl>
                <dt>NIK</dt>
                <dd><input class="form-control form-control-md" type="text" name="no_pengenal"   value="<?php echo $_SESSION['user']['no_pengenal']; ?>"  required></dd>
              </dl></li>
              <li class="pb-1"><dl>
                <dt>Nomor Ponsel</dt>
                <dd><input class="form-control form-control-md" type="text" name="nope" value="<?php echo $_SESSION['user']['no_hp']; ?>"  required></dd>
              </dl></li>
              <li class="pb-1"><dl>
                <dt>Email</dt>
                <dd><input class="form-control form-control-md" type="email" name="email" value="<?php echo $_SESSION['user']['email']; ?>"  required></dd>
              </dl></li>
              <li class="pb-1"><dl>
                <dt>Tanggal Lahir</dt>
                <dd><input class="form-control form-control-md" type="date" id="date" value="<?php echo $_SESSION['user']['tanggal_lahir']; ?>" name="tanggal_lahir" required></dd>
              </dl></li>
            </ul>
      	</div>
      </form>
